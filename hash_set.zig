...
test "HashSet" {
    ...
    try testing.expectEqual(@as(usize, 1), str_hash_set.count());
    _ = str_hash_set.delete("x");
    try testing.expectEqual(@as(usize, 0), str_hash_set.count());
    ...
    try testing.expectEqual(@as(usize, 3), str_hash_set.count());
    try testing.expect(str_hash_set.exists("x"));
    try testing.expect(str_hash_set.exists("y"));
    try testing.expect(str_hash_set.exists("z"));
    try testing.expect(!str_hash_set.exists("a"));

    var int_hash_set = AutoHashSet(u8).init(std.testing.allocator);
    defer int_hash_set.deinit();

    try int_hash_set.put(1);
    try testing.expectEqual(@as(usize, 1), int_hash_set.count());
    _ = int_hash_set.delete(1);
    try testing.expectEqual(@as(usize, 0), int_hash_set.count());
    ...
    try testing.expectEqual(@as(usize, 3), int_hash_set.count());
    try testing.expect(int_hash_set.exists(1));
    try testing.expect(int_hash_set.exists(2));
    try testing.expect(int_hash_set.exists(3));
    try testing.expect(!int_hash_set.exists(4));

    var i: usize = 0;
    var iterator = int_hash_set.iterator();
    while (iterator.next()) |v| {
        try testing.expect(v == 1 or v == 2 or v == 3);
        i += 1;
    }
